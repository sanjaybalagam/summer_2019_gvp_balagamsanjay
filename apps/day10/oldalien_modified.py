...
angle=[(2.5)*x for x in [_ for _ in range(18)]]
...
class Score(Actor):
    ...
    def __init__(self):
        ...
        self.print_string = 'SCORE' + str(Score.score) + "    "
        self.score_board_font = self.font.render(self.print_string, True, Score.white)
        self.rect = self.score_board_font.get_rect()
        self.rect_centre_squares=[(100,460),(100,30),(500,30),(500,460)]
        self.rect.center =self.rect_centre_squares[Score.i]

    def draw(self,screen):
        self.print_string = 'SCORE' + str(Score.score) + "    "
        self.rect.center = self.rect_centre_squares[self.i]
        self.score_board_font = self.font.render(self.print_string, True, Score.white)
        r=screen.blit(self.score_board_font,self.rect)
        dirtyrects.append(r)
...
'''
    def __init__(self, player):
        Actor.__init__(self, Img.shot)
        self.moveright = bullet_direction_i
        if self.moveright <=9:
            self.image = pygame.transform.rotate(Img.shot, angle[self.moveright]-90  )
        else:
            self.image= pygame.transform.rotate(Img.shot , 90- angle[self.moveright])
        self.rect.centerx = player.rect.centerx
        self.rect.top = player.rect.top - 10
'''
    def update(self):
        ...
        if self.moveright <9:
            self.rect.right+=(9-self.moveright)
        else:
            self.rect.right-=(self.moveright - 9)
...
def main():
    ...
    while player.alive or explosions:
        ...
        # Move the player
        direction = keystate[K_l] - keystate[K_j]
        player.move(direction)

        '''
        if keystate[K_LEFT]:
            bullet_direction_i +=1

        if keystate[K_RIGHT]:
            bullet_direction_i-=1

        if bullet_direction_i <0 :
            bullet_direction_i=0

        if bullet_direction_i > 17:
            bullet_direction_i = 17
        '''
        # Create new shots
        ...
        for shot in shots:
            ...
            if hit != -1:
                ...
                alien = aliens[hit]
                explosions.append(Explosion(alien))
                score_board.erase(screen, background)
        ...
        # Draw everybody
        for actor in [player] + aliens  + explosions + shots:
            actor.draw(screen)
